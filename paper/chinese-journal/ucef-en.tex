...
%  Chinese Journal Format (计算机学报 style) — English Version
%
%  Compile:
%    pdflatex ucef-en && bibtex ucef-en && pdflatex ucef-en && pdflatex ucef-en
% ══════════════════════════════════════════════════════════════════════════
...
% ── Page layout (计算机学报 style: double-column, A4) ──────────────────────
\geometry{
...
% ── Title ─────────────────────────────────────────────────────────────────
\title{\LARGE \ucef{}: Universal Context Extension Framework---Breaking the Context Barrier with Hyperbolic Geometry and Quantum-Inspired Selection}

\author{
  \textbf{Honglin He}\\[4pt]
  \small hehonglin525@gmail.com\\[2pt]
  \small Project Page: \url{https://github.com/ViewWay/UCEF}
}
...
\medskip
\noindent\textbf{Keywords:} context extension, hyperbolic geometry, quantum-inspired retrieval, adaptive compression, large language models
\end{abstract}
...
\emph{Position encoding scaling} methods~\cite{peng2024yarn} modify model
internals to accept longer sequences but cannot extend beyond approximately
$4\times$ the native window without quality degradation.
...
(1)~\textbf{hyperbolic retrieval} using Poincar\'{e} ball embeddings that
naturally reflect hierarchical relationships~\cite{nickel2017poincare},
yielding 35\% improvement in retrieval precision;
(2)~\textbf{quantum-inspired selection} via density matrices encoding
inter-document correlations~\cite{vanrijsbergen2004,zuccon2009}, improving
accuracy by 30.8\%~\cite{kuznetsov2026qisa}; and
(3)~\textbf{adaptive compression} guided by a closed-loop quality feedback
system monitoring four quality dimensions.
...
improvement for hierarchical knowledge bases.

\subsection{Quantum-Inspired Information Retrieval}

Van Rijsbergen~\cite{vanrijsbergen2004} established the theoretical
foundation for quantum probability in information retrieval.  Zuccon
et~al.~\cite{zuccon2009} proposed the Quantum Probability Ranking Principle.
Kuznetsov et~al.~\cite{kuznetsov2026qisa} demonstrated 30.8\% accuracy
improvement over classical transformers through quantum-inspired attention
mechanisms.  Sasaki and Ueda~\cite{sasaki2023quantumctx} applied density
matrix ranking to document retrieval.
...
reducible to $O(\log n)$ using HNSW indexing in hyperbolic space.
...
\emph{Adaptive}: automatically selects the strategy based on $\gamma$.
...
distortion than Euclidean embeddings.
